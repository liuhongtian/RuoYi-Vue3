    ...
    >
      ...
      <template v-slot:footer>
        <div class="dialog-footer">
          ...
          <el-button size="small" type="primary" @click="checkUserComplete"
            >确 定</el-button
          >
        </div>
      </template>
    ...
    >
      ...
      <template v-slot:footer>
        <div class="dialog-footer">
          ...
          <el-button size="small" type="primary" @click="checkRoleComplete"
            >确 定</el-button
          >
        </div>
      </template>
    ...
    >
      ...
      <template v-slot:footer>
        <div class="dialog-footer">
          ...
          <el-button size="small" type="primary" @click="checkExpComplete"
            >确 定</el-button
          >
        </div>
      </template>
  ...
  methods: {
    ...
    getUserList(val, key) {
      if (StrUtil.isNotBlank(val)) {
        ...
        if ('assignee' === key) {
          this.selectData[key] = newArr.find(
            (item) => item.userId.toString() === val
          ).userId
        } else {
    ...
    unique(arr, code) {
      const res = new Map()
      return arr.filter(
        (item) => !res.has(item[code]) && res.set(item[code], 1)
      )
    },
