<template>
  <div class="panel-tab__content">
    ...
    <el-drawer v-model="listenerFormModelVisible" title="任务监听器" size="480px" append-to-body destroy-on-close>
      <el-form :model="listenerForm" size="small" label-width="110px" ref="listenerFormRef" @submit.prevent>
              ...
              <el-tooltip placement="top">
                <template v-slot:content>
                  <div>
                    ...
                    <br />assignment（指派）：当任务已经指派给某人时触发。请注意：当流程执行到达用户任务时，
                    <br />
                    在触发create事件之前，会首先触发assignment事件。这顺序看起来不太自然，
                    <br />
        ...
        <el-form-item v-if="listenerForm.listenerType === 'delegateExpressionListener'" label="代理表达式"
          prop="delegateExpression" :rules="{ required: true, trigger: ['blur', 'change'] }">
          <el-input v-model="listenerForm.delegateExpression" clearable />
        </el-form-item>
        <template v-if="listenerForm.listenerType === 'scriptListener'">
          <el-form-item label="脚本格式" prop="scriptFormat" key="listener-script-format" :rules="{
            required: true,
            trigger: ['blur', 'change'],
            message: '请填写脚本格式',
          }">
          ...
          <el-form-item label="脚本类型" prop="scriptType" key="listener-script-type" :rules="{
            required: true,
            trigger: ['blur', 'change'],
            message: '请选择脚本类型',
          }">
          ...
          <el-form-item v-if="listenerForm.scriptType === 'inlineScript'" label="脚本内容" prop="value" :rules="{
            required: true,
            trigger: ['blur', 'change'],
            message: '请填写脚本内容',
          }">
          ...
          <el-form-item v-if="listenerForm.scriptType === 'externalScript'" label="资源地址" prop="resource" :rules="{
            required: true,
            trigger: ['blur', 'change'],
            message: '请填写资源地址',
          }">
        ...
        <template v-if="listenerForm.event === 'timeout'">
          ...
          " label="定时器" prop="eventTimeDefinitions" :rules="{
              required: true,
              trigger: ['blur', 'change'],
              message: '请填写定时器配置',
            }">
    ...
    <!-- 注入西段 编辑/创建 部分 -->
    <el-dialog title="字段配置" v-model="listenerFieldFormModelVisible" width="600px" append-to-body
      destroy-on-close>
  ...
  methods: {
    resetListenersList() {
      ...
      this.elementListenersList = this.bpmnElementListeners.map((listener) =>
        this.initListenerType(listener)
      )
      $emit(this, 'getTaskListenerCount', this.elementListenersList.length)
    },
