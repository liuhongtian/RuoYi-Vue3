...
const { is: is$f, isAny: isAny$5 } = require$$0
...
var endEventRequired = function () {
  function hasEndEvent(node) {
    ...
    return flowElements.some((node) => is$f(node, 'bpmn:EndEvent'))
  }

  function check(node, reporter) {
    if (!isAny$5(node, ['bpmn:Process', 'bpmn:SubProcess'])) {
      return
    }
...
var fakeJoin = function () {
  function check(node, reporter) {
    if (!isAny$4(node, ['bpmn:Task', 'bpmn:Event'])) {
      return
    }
...
const { is: is$d, isAny: isAny$3 } = require$$0
...
function uniqueBy(extractor) {
  ...
  for (
    var _len = arguments.length,
      collections = new Array(_len > 1 ? _len - 1 : 0),
      _key = 1;
    _key < _len;
    _key++
  ) {
...
const { isAny: isAny$2, is: is$a } = require$$0
...
var startEventRequired = function () {
  function hasStartEvent(node) {
    ...
    return flowElements.some((node) => is$5(node, 'bpmn:StartEvent'))
  }
...
var checkNodeUser = function () {
  function check(node, reporter) {
    if (is(node, 'bpmn:Task')) {
      ...
      const buttons =
        node.extensionElements?.values?.filter((ex) => {
          return ex.$type === `flowable:Buttons`
        }) ?? []
      const selectButtons = buttons.reduce(
