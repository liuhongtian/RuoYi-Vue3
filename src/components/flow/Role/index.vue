    ...
    >
      <el-table-column width="55" align="center">
        <template v-slot="scope">
          ...
          <el-radio v-model:value="radioSelected" :label="scope.row.roleId">{{
            ''
          }}</el-radio>
        </template>
      </el-table-column>
...
export default {
  ...
  methods: {
    ...
    handleMultipleRoleSelect(selection) {
      ...
      $emit(this, 'handleRoleSelect', idList.join(','), nameList.join(','))
    },
    // 单选框选中数据
    handleSingleRoleSelect(selection) {
      this.radioSelected = selection.roleId
      const roleName = selection.roleName
      $emit(this, 'handleRoleSelect', this.radioSelected.toString(), roleName)
    },
    /** 搜索按钮操作 */
    handleQuery() {
