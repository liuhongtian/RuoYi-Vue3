    ...
    <el-row :gutter="10" class="mb8">
      ...
      <el-col :span="1.5">
        <el-button
          ...
          icon="Download"
          @click="handleExport"
          v-hasPermi="['temp:WXMSG:export']"
        >导出</el-button>
      </el-col>
      <right-toolbar v-model:showSearch="showSearch" @queryTable="getList"></right-toolbar>
    </el-row>

    <el-table v-loading="loading" :data="WXMSGList" @selection-change="handleSelectionChange" @sort-change="handleSortChange">
      <el-table-column type="selection" width="55" align="center" fixed="left" />
      <el-table-column label="操作" align="center" class-name="small-padding fixed-width" fixed="left">
        <template #default="scope">
          <el-button link type="primary" icon="Edit" @click="handleUpdate(scope.row)" v-hasPermi="['temp:WXMSG:edit']">修改</el-button>
          <el-button link type="primary" icon="Delete" @click="handleDelete(scope.row)" v-hasPermi="['temp:WXMSG:remove']">删除</el-button>
        </template>
      </el-table-column>
      ...
      <el-table-column label="发送状态" align="center" :sort-orders="['descending', 'ascending']" sortable="custom" prop="errcode" />
...
import { listWXMSG, getWXMSG, delWXMSG, addWXMSG, updateWXMSG } from "@/api/temp/WXMSG";
...
function getList() {
  loading.value = true;
  listWXMSG(queryParams.value).then(response => {
    WXMSGList.value = response.rows;
...
/** 提交按钮 */
function submitForm() {
  proxy.$refs["WXMSGRef"].validate(valid => {
    if (valid) {
      if (form.value.pkId != null) {
        updateWXMSG(form.value).then(response => {
          proxy.$modal.msgSuccess("修改成功");
          open.value = false;
          getList();
        });
      } else {
        addWXMSG(form.value).then(response => {
...
function handleDelete(row) {
  ...
  proxy.$modal.confirm('是否确认删除待发送消息编号为"' + _pkIds + '"的数据项？').then(function() {
    return delWXMSG(_pkIds);
  }).then(() => {
    getList();
...
function handleExport() {
  proxy.download('temp/WXMSG/export', {
    ...queryParams.value
  }, `WXMSG_${new Date().getTime()}.xlsx`)
}
